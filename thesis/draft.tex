...
Not surprisingly, employing the GCT model with 2 substages for both generating and fitting incidence data also resulted in a close match, as indicated by an RMSLE of 2.34\% (Figure \ref{sametypeFittingG}.A), and parameters post-fitting were close to the exact values (Figure \ref{sametypeFittingG}.B). Unlike the case with the LCT model, when the structure of the fitting model was altered from 2 substages to 6, the model still exhibited a close match with the original in terms of both shape (Figure \ref{sametypeFittingG}.C) and parameters (Figure \ref{sametypeFittingG}.D). This indicates that the GCT model's fitting performance is minimally impacted by changes in the model structure.
...
% Here
%%%%%%%%
To more clearly illustrate the performance differences between the LCT and GCT models, fittings with various model structures were conducted to assess changes in percentage error. The percentage error from the fitting result, where the model generating the incidence data and the model for fitting are exactly the same, has been used as the baseline (red line). As shown in Figure X, when using the same input data, varying the fitting model shape causes the LCT model to experience a surge in RMSLE values, while the GCT model maintains its error rate close to the baseline. This result, once again, confirms that the structure of the GCT model merely affects the fitting results, indicating that changes in structure to optimize model performance are no longer necessary.

To inllstriate more clear about the performance difference between LCT and GCT, fittings using different model structure have been done to see how the error would change. As shown in Figure x, when treating the percentage error from the the fitting result where the model to generate the incidence data and the model for fitting are exactly the same, varying the fitting model shape by using a different number of substages from the model used to generate the data causes the LCT model to experience a surge in RMSLE values, while the GCT model maintains its error rate close to the baseline. This result, once again, confirmed that the structure of the GCT model merely affect the model fitting result, showing that changing in structure in order to find the optimized performance of the model is no longer necessary.
